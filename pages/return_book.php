...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($user_result->num_rows > 0) {
        ...
        $issue_query = "SELECT * FROM issued_books WHERE user_id = '$user_id' AND isbn = '$isbn'";
        $issue_result = $conn->query($issue_query);

        if ($issue_result->num_rows > 0) {
            $sql = "DELETE FROM issued_books WHERE user_id = '$user_id' AND isbn = '$isbn'";
            $update_book = "UPDATE books SET status = 'available' WHERE isbn = '$isbn'";
